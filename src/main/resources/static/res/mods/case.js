  ...
  var active = {
    ...
    push: function (div) {
      layer.open({
        ...
        title: '提交案例'
        ,
        area: (device.ios || device.android) ? ($(window).width() + 'px')
            : '660px'
        ,
        ...
        success: function (layero, index) {
          ...
          upload.render({
            ...
            , done: function (res) {
              if (res.status == 0) {
                image.val(res.url);
                preview.html('<a href="' + res.url
                    + '" target="_blank" style="color: #5FB878;">封面已上传，点击可预览</a>');
              } else {
        ...
        layer.open({
          ...
          , content: function () {
            layui.each(res.data, function (_, item) {
              html += '<li><a href="/u/' + 168 * item.id
                  + '/" target="_blank" title="' + item.username
                  + '"><img src="' + item.avatar + '"></a></li>'
            });
